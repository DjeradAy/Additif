...
class AdditifModel:
    ...
    def show_model_predictions(self, y_train_predicted=None, y_test_predicted=None):
       ...
       #TFD
        ...
        ok = y_test_predicted[:, 0] + 1.96 * numpy.std(y_test_predicted[:, 0])
        okk = y_test_predicted[:, 0] - 1.96 * numpy.std(y_test_predicted[:, 0])

        plt.fill_between(self.data_preparation_object.dataset_df["Years"].iloc[int(len(self.data_preparation_object.dataset_df)*0.75):], okk, ok, color="lightgray", alpha=0.5, label="95% Confidence Interval")
